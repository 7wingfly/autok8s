...
There a quite a few paramters you can pass into the script. At the very least you will need to provid an IP address for the server and the IP range for the load balancer. 

```
./setup_master_node.sh --ip-address 192.168.0.10 --k8s-load-balancer-ip-range 192.168.0.20-192.168.0.29
```

Check out the Master Node Parameters [document](https://github.com/7wingfly/autok8s/tree/main/ParametersMasterNode.md) for all available options.
