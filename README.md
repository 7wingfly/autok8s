# <t style="color: #dd54ffff">Auto</t><t style="color: #49aeddff">k8s</t> (Automatic Kubernetes)
...
This will be of interest to anyone wanting to try out Kubernetes for the first time without spending any money, k8s enthusiasts/professionals that want a cluster at home for free, or those who are familiar with cloud offerings such as EKS or AKS but would like to learn more about how Kubernetes works under the hood. This may also help those looking to create an on-premise or self-managed cluster in a professional environment.
...
This project aims to fully automate the installation and configuration of a Kubernetes control-plane node along with the worker nodes with no more than one command per node. It includes manifests and Helm charts for pod networking (CNI), a load-balancer, persistent storage, and other common features and tools.

With built-in support for [Flux CD](https://fluxcd.io/), you can leverage the power of GitOps to automatically deploy all of your workloads and configurations as soon as the cluster is ready.
...
Once installation is complete the following message will be shown detailing the command for joining worker nodes to your cluster using the `setup_worker_node.sh` script as well as some other tips and useful information.
