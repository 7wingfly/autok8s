...
echo -e '\e[35m  Kubernetes Cleanup Script:\e[36m Control-Plane Edition\e[0m\n'
...
if [ -n "${SUDO_USER:-}" ] && [ -d "/home/$SUDO_USER/.kube" ]; then
...
# Remove Cilium and Hubble CLIs if present

if command -v cilium &>/dev/null; then
  echo -e "\033[32mRemoving Cilium CLI\033[0m"
...
echo -e "\033[32mRemoving Helm\033[0m"
...
if dpkg -l nfs-kernel-server &>/dev/null 2>&1; then
  echo -e "\033[32mRemoving NFS server\033[0m"
  systemctl stop nfs-kernel-server 2>/dev/null || true
  apt-get purge -qqy $APT_LOCK nfs-kernel-server 2>/dev/null || true
  echo -e "\033[33mNote:\033[0m NFS share directories have not been removed."
fi
...
if dpkg -l samba &>/dev/null 2>&1; then
  echo -e "\033[32mRemoving Samba\033[0m"
  systemctl stop smbd nmbd 2>/dev/null || true
  apt-get purge -qqy $APT_LOCK samba samba-common 2>/dev/null || true
  echo -e "\033[33mNote:\033[0m SMB share directories have not been removed."
fi
...
if [ -n "$LATEST_FSTAB_BACKUP" ]; then
  ...
  # No backup found — uncomment any swap lines that were commented out
